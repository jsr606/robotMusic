...
void dubregn() { 
  if (state == 0) {
    ...
    kaossPad.setEffects(0.2, 0.8);
    kaossPad.tapTempo(76, 8);
  ...
  if (state == 1) {
    if (randomTime) {
      randomInsertStep();
      if (millis() % 3 == 0) kaossPad.setEffects(random(0,1), 1);
    }
  ...
  if (state == 15) {
    ...
    if ((songLength/1000)>(5*60)) {
      println("song is more than 5 minutes old, ending it");
      state = 16;
    }
  }
